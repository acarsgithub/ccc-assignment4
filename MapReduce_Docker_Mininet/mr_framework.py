            ...
            if (args['op'] == "workers_up"):
                ...
                for item in map_resp:
                    ...
                    map_file.write (
                        ...
                        + str(map_resp[item]['work_count']) + str (",") 
                        + str(map_resp[item]['load']) + str (",") 
                        + str(map_resp[item]['load_count']) + "\n"
                    )
                # close the file
                map_file.close ()
...
class MR_Framework ():
    ...
    def distribute_map_tasks (self):
        ...
        couch = couchdb.Server('http://admin:hi@129.114.26.89:5984/')
        db = couch['testing']
    ...
    def shuffle_func (self):
        ...
        for i in range (self.M):
            ...
            groups = []
            uniquekeys = []
            for k, g in itertools.groupby (energylist, key=operator.itemgetter (0)):
